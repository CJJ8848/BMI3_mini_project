...
def settfrecords(cwd,outfile):
    ...
    for index, name in enumerate(classes):
        ...
        for img_name in os.listdir(class_path):
            ...
            # index 0 1 2... are set for each class as "label",
            # images are saved as binary format in the "img_raw" feature
...
def read_and_decode(filename):
    # generate a queue to receive the tfrecords data
    ...
    # Throws the Img tensor in the flow
    img = tf.decode_raw(features['img_raw'], tf.uint8)
    # Reshape the data to a three channels 224 * 224 images
    img = tf.reshape(img, [224, 224, 3])
    # Subtracting the image mean to increase the image sharpness,
...
epoch = 0 # epoch =8
batch_size = 8 # the size of one batch
learning_rate = 0.0001 # laerning rate
...
def weight_variable(shape):
    ...
    initialweight = tf.truncated_normal(shape, stddev = 0.02) # mean = 0, standard deviation = 0.02
    return tf.Variable(initialweight) # initial the variable
#initial bias
# shape: the dimension of the output tensor
def bias_variable(shape):
    initialbias = tf.constant(0.0, shape=shape) # Create a constant with dimension shape and value 0 in TensorFlow
    return tf.Variable(initialbias)
...
def conv(x, W):
    return tf.nn.conv2d(x, W, strides=[1,1,1,1], padding='SAME') # strides = [1,1,1,1], padding = 'SAME' means edge filling (conv layer 's shape is still 224x224)
...
if __name__ == '__main__':
    ...
    output_conv1 = tf.nn.relu(conv(x, W_conv1) + b_conv1)# relu as an activity function
    output_pool1 = max_pool_4x4(output_conv1)# 4x4 max pooling layer to reduce the parameters
    ...
    output_conv2 = tf.nn.relu(conv(output_pool1, W_conv2) + b_conv2)
    output_pool2 = max_pool_4x4(output_conv2)

    #fully connected layer
    reshape = tf.reshape(output_pool2,[batch_size, -1])
    dim = reshape.get_shape()[1].value
    W_fc1 = weight_variable([dim, 1024])# 1024 neurons in the fully connected layer
    b_fc1 = bias_variable([1024])
    output_fc1 = tf.nn.relu(tf.matmul(reshape, W_fc1) + b_fc1)#activate
    #dropout
    keep_prob = tf.placeholder(tf.float32)
    output_fc1_drop = tf.nn.dropout(output_fc1, keep_prob)# keep_prob means the memory keeping probability (used to control the overfit)
    #softmax predict the probability of belonging to each class
    # the output layer of fullu connected network
    ...
    txt_save_path = dirname(__file__) + '/2layers_accuracy_for_the_best_model.txt'
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(classes)):
            fpr[i], tpr[i], thresholds = metrics.roc_curve(group[:, i], prob[:, i])
            roc_auc[i] = metrics.auc(fpr[i], tpr[i])

        # Compute micro-average ROC curve and ROC area（方法二）
        fpr["micro"], tpr["micro"], _ = metrics.roc_curve(group.ravel(), prob.ravel())
        ...
        # with open("new_accuracy_for_ROC.txt", 'a') as file_handle:
        #     file_handle.write('\n')
        #     file_handle.write(
